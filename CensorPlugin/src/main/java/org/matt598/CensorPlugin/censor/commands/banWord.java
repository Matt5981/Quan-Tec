...
public class banWord implements Command {
    private final CensorManager censorManager;

    public banWord(CensorManager censorManager){
    ...
    public void run(MessageReceivedEvent event){
        ...
        if(event.getMessage().getContentRaw().split(" ").length < 2){
            event.getChannel().sendMessage("**Error!** This commands requires at least 1 argument.").queue();
            ...
            responseString.append("**Banned the following words:** ");

            for(String candidate : toRemove){
                censorManager.addWord(event.getGuild().getId(), candidate);
